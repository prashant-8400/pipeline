...
class Configuration:
    def __init__(self,spark_config_loc,datasetName, dataset_path):
        ...
        #self.ingest_source = self.jsonData['dataset']['source']['data-location']
        #self.ingest_dest = self.jsonData['dataset']['destination']['data-location']
        
        # Ingesting file format to raw zone from landing zone
        
        #self.ingest_source_format = self.jsonData['dataset']['source']['file-format']
        #self.ingest_dest_format = self.jsonData['dataset']['destination']['file-format']
        
        # Reading data location to be read from raw zone
        
        self.mask_source = self.jsonData['masked-cast-dataset']['source']['data-location'] + dataset_path
        self.mask_dest = self.jsonData['masked-cast-dataset']['destination']['data-location'] + ("/".join(dataset_path.split("/")[:len(dataset_path.split("/"))-1]))
        
        # Reading file format to be read from raw zone
        
        self.mask_source_format = self.jsonData['masked-cast-dataset']['source']['file-format']
        self.mask_dest_format = self.jsonData['masked-cast-dataset']['destination']['file-format']

        # Reading masking-columns required to be done
        
        self.mask_cols = self.jsonData['masked-cast-dataset']['masking-cols']
        ...
        self.transformation_cols = self.jsonData['masked-cast-dataset']['transformation-cols'] 

        # Reading partition-columns required to be done
        self.partition_cols = self.jsonData['masked-cast-dataset']['partition-cols'] 
