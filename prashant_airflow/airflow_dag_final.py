...
# Copying Files/Folder from one bucket to another
...
def copy_data(**kwargs):
    ...
    source_path_list = (jsonData['dataset']['source']['data-location']+dataset_path).replace(":","").split("/")
    dest_path_list = (jsonData['dataset']['destination']['data-location']+dataset_path).replace(":","").split("/")
...
# PreValidation :- Checking same data is available or not in landing and raw zone
...
def pre_validation(**kwargs):
    ...
    df_landingzone = pq.ParquetDataset(jsonData['dataset']['source']['data-location']+dataset_path, filesystem=s3).read_pandas().to_pandas()
    df_rawzone = pq.ParquetDataset(jsonData['dataset']['destination']['data-location']+dataset_path, filesystem=s3).read_pandas().to_pandas()
...
# Creates an EMR cluster and submitting job to Livy via Post method
...
def livy_submit(**kwargs):
    ...
    cluster_id = emr.create_cluster(region_name=region, cluster_name='Prashat_Cluster', num_core_nodes=1)
...
# PostValidation :- Checking the no of records and column datatype after emr_spark job 
...
def post_validation(**kwargs):
    ...
    df_rawzone = pq.ParquetDataset(jsonData['masked-cast-dataset']['source']['data-location']+("/".join(dataset_path.split("/")[:len(dataset_path.split("/"))-1])), filesystem=s3).read_pandas().to_pandas()
    df_stagingzone = pq.ParquetDataset(jsonData['masked-cast-dataset']['destination']['data-location']+("/".join(dataset_path.split("/")[:len(dataset_path.split("/"))-1])), filesystem=s3).read_pandas().to_pandas()
    ...
    if df_stagingzone[df_stagingzone.columns[0]].count() != 0: # df.columns[0] = 1st column
        ...
        # Datatype validation
        for i in jsonData['masked-cast-dataset']['transformation-cols']:
            if jsonData['masked-cast-dataset']['transformation-cols'][i].split(',')[0] == "DecimalType":
                if isinstance(df_stagingzone[i][0],decimal.Decimal) & (str(abs(decimal.Decimal(df_stagingzone[i][0]).as_tuple().exponent)) == jsonData['masked-cast-dataset']['transformation-cols'][i].split(',')[1]):
                    print('Datatype matches for '+ i)
                else:
                    raise TypeError("Datatype Mismatch in "+i)
            if jsonData['masked-cast-dataset']['transformation-cols'][i].split(',')[0] == "ArrayType-StringType" or jsonData['masked-cast-dataset']['transformation-cols'][i].split(',')[0] == "StringType":
...
dag_args = {
    'owner': 'prashant_airflow',
    'depends_on_past': False,
    'start_date': datetime(2022,2,13),
    'email': ['prashant.anil@tigeranalytics.com'],
    'email_on_failure': True,
    'retry_delay': timedelta(minutes=3)
    }

dag = DAG(
    dag_id='final_dag_project',
    start_date=datetime(2022,2,13),
